...
Solucao Perturbacao(Solucao& s, Data& problem_info, vector<vector<Subsequence>>& matrix){
    ...
    size_t max_tamanho = max((size_t)2, (size_t)ceil(problem_info.getDimension()/10.0));
    ...
    UpdateAllSubseq(s_linha, matrix, problem_info);
    s_linha.valorObj = matrix[0][s.sequencia.size()-1].C;
