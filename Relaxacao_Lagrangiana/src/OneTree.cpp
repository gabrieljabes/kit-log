...
OneTree SubgradientMethod(double& ub, vector<double>& lambda, vector<vector<double>>& custos){
    int dim = custos.size();
    ...
    while(epsilon > epsilon_min && !best_tree.is_tour){
        ...
        for(int i = 0; i < dim; i++){
            for(int j = i + 1; j < dim; j++){
                custos_linha[i][j] = custos[i][j] - lambda[i] - lambda[j];
                custos_linha[j][i] = custos_linha[i][j];
            }
        }

        OneTree tree = solve1Tree(lambda, custos_linha);
        ...
        if(tree.is_tour || tree.LB >= ub) break;
        // somatorio dos nos
        double sum = 0;
        for(int i = 0; i < dim; i++)
